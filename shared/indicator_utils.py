...
def compute_all_indicators(candles_df, prefix=""):
    ...
    # WHY (May 2026 — Fix B): added period 21 to match the plain `adx`
    #      group's period list. Rules generated by the system reference
    #      `*_mt5_adx_21` which was previously unbuilt → "MISSING"
    #      feature check → rule matched 0 trades.
...
def compute_indicators(df, only=None, prefix="", skip_smart=False):
    ...
    if only is None or 'mt5_adx' in only or 'adx' in only:
        # WHY (May 2026 — Fix B): added period 21 to match the plain
        #      `adx` group's period list. See the equivalent block at
        #      ~L290 for the full rationale.
