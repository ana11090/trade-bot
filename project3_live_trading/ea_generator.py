...
def generate_ea(
    ...
    floor_to_min_lot=True,   # True=floor to minLot (Option A, matches Python); False=skip (Option B, risk-honest)
   ...
   {{
    ...
    if exit_class in ('ATRBased', 'ATROnly', 'ATRTrailing',
        ...
        if exit_class == 'ATRBreakevenTrail':
            ...
            # WHY (May 2026): exit_management below references MaxHoldCandles for the
            #      time-limit cap. The original fix used = which clobbered the shared
            #      ATR inputs (SL_ATR_Mult, TP_ATR_Mult, ATR_Period) set in the block
            #      above, producing undeclared-identifier compile errors.
            # CHANGED: May 2026 — declare MaxHoldCandles for ATRFixedSLTP
            # CHANGED: June 2026 — APPEND to ATR inputs instead of overwriting them
            exit_inputs = exit_inputs + (
                f'input int    MaxHoldCandles  = {max_candles};               // Force close after N candles (0=disabled)\n'
            )
            exit_globals = (
            ...
            exit_management = (
                ...
                f'{_psar_flip}'
                f'         {{\n'
                f'            Print("[PSAR-DIAG]","|entry_bar=",g_entryBarIndex,"|exit_bar=",Bars(_Symbol,{mql_period}),"|hold=",_barsHeld,"|reason=PSARFlipExit","|psar=",DoubleToString(_psarBuf[0],_Digits),"|close1=",DoubleToString(_psarClose,_Digits),"|t_exit=",TimeToString(iTime(_Symbol,{mql_period},0),TIME_DATE|TIME_MINUTES));\n'
                ...
                f'      {{\n'
                f'         Print("[PSAR-DIAG]","|entry_bar=",g_entryBarIndex,"|exit_bar=",Bars(_Symbol,{mql_period}),"|hold=",_barsHeld,"|reason=PSARTimeExit","|t_exit=",TimeToString(iTime(_Symbol,{mql_period},0),TIME_DATE|TIME_MINUTES));\n'
...
'''
    ...
    # CHANGED: June 2026 — None-safe: dict.get(k, 0) returns None when key exists with value
    #   None, so coerce explicitly with `or 0` before formatting.
    _bs_wr = (base_stats.get('win_rate') or 0) * 100
    _bs_pf = base_stats.get('profit_factor') or 0
    _bs_tp = base_stats.get('total_pips') or 0
    _vr.append(f"  Backtest: WR {_bs_wr:.1f}%, PF {_bs_pf:.2f}, {_bs_tp:+,.0f} pips")
    ...
    _floor_mql = "true" if floor_to_min_lot else "false"
...
input bool   FloorToMinLot      = {_floor_mql};             // true=take trade at minLot if risk-lots<min; false=skip (risk-honest)
...
{{
   ...
   // WHY: FloorToMinLot controls the lot-parity policy between EA and Python.
   //   true  (Option A): floor to minLot — trade executes, real risk exceeds RiskPercent.
   //   false (Option B): skip trade — risk-honest; surfaces rules untradeable at this account size.
   //   Both engines must read the same setting or PnL can never reconcile.
   // CHANGED: June 2026 — flagged instead of hard-coded skip
   if(lots < minLot)
   {{
      if(FloorToMinLot)
      {{
         Print("[LOTS] Risk requested ", DoubleToString(lotsRaw, 4),
               " lots < minLot=", minLot, " — flooring to minLot (real risk EXCEEDS RiskPercent).");
         lots = minLot;
      }}
      else
      {{
...
def _generate_tradovate(win_rules, exit_name, exit_params, symbol, magic_number,
    ...
    # CHANGED: June 2026 — None-safe pre-computed values for the header f-string
    _hdr_wr = (base_stats.get("win_rate") or 0) * 100
    _hdr_tp = base_stats.get("total_pips") or 0
    code = f'''\
#!/usr/bin/env python3
"""
...
Backtest: WR {_hdr_wr:.1f}%, {_hdr_tp:.0f} net pips
