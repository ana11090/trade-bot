...
def _start_optimization():
    ...
    def _worker():
        try:
            ...
            globals()['_opt_page'] = 1            # fresh run → start at page 1
            globals()['_opt_page_total'] = None   # force pager to re-init
...
def _ensure_opt_sim_metrics(cand):
    """Compute & cache sim-derived metrics on a candidate so the optimizer-section
    filters (windows / passed% / max consec DD) have real values to compare.
    Runs simulate_challenge ONCE per candidate (cached on _oc_sim_cached). Safe to
    call repeatedly. WHY: these fields were never stored, so those filters did nothing.
    CHANGED: June 2026 — self-computing filter metrics.
    """
    try:
        if cand.get('max_consecutive_dd_breaches') is not None:
            return
        sim = cand.get('_oc_sim_cached')
        if sim is None:
            from project2_backtesting.strategy_validator import _trades_to_df
            from shared.prop_firm_simulator import simulate_challenge
            trades = cand.get('trades', [])
            # resolve firm/challenge the same way the cards do
            try:
                _fid, _cid = _resolve_firm_challenge(cand, 10000)
            except Exception:
                _fid = _cid = None
            if not (trades and _fid and _cid):
                return
            ep = cand.get('exit_params') or {}
            sl = float(ep.get('sl_pips', 150) or 150)
            pv = float(cand.get('pip_value_per_lot', 1.0) or 1.0)
            try:
                _risk = float(_opt_risk_var.get()) if globals().get('_opt_risk_var') else 0.3
            except Exception:
                _risk = 0.3
            df = _trades_to_df(trades, _risk, sl, pv, 10000)
            sim = simulate_challenge(trades_df=df, firm_id=_fid, challenge_id=_cid,
                                     account_size=10000, mode='sliding_window',
                                     simulate_funded=False, risk_per_trade_pct=_risk,
                                     default_sl_pips=sl, pip_value_per_lot=pv, symbol='XAUUSD')
            cand['_oc_sim_cached'] = sim
        if sim is not None:
            from project2_backtesting.strategy_refiner import max_consecutive_dd_breaches
            cand['max_consecutive_dd_breaches'] = max_consecutive_dd_breaches(
                getattr(sim, 'individual_results', None))
            _p = sim.eval_pass_count
            _t = sim.eval_pass_count + sim.eval_fail_count
            cand['win_pass_passed'] = _p
            cand['win_pass_total']  = _t
            cand['win_pass_rate']   = (_p / _t) if _t else 0.0
    except Exception:
        pass
...
def _opt_variant_passes(cand):
    ...
    try:
        ...
        # WHY: the windows / passed% / max-consec-DD filters need sim-derived numbers
        #      (win_pass_total, win_pass_rate, max_consecutive_dd_breaches). These were
        #      NEVER stored on candidates, so the filters silently did nothing. Compute
        #      them ONCE here (cached on the candidate) when any of those filters is on.
        # CHANGED: June 2026 — self-computing filter so consec-DD / windows actually work
        if (_mwin > 0 or _mp > 0 or _mc > 0) and cand.get('max_consecutive_dd_breaches') is None:
            _ensure_opt_sim_metrics(cand)
...
def _rerender_opt_run_cards():
    ...
    try:
        ...
        if not kept:
            ...
            if globals().get('_opt_status_lbl'):
                _opt_status_lbl.configure(text="✓ Filter applied — 0 variants match", fg=AMBER)
            return
        # CHANGED: June 2026 — PAGINATION: 50 cards per page so all matches are
        #          accessible (pages 1,2,3…) but never more than 50 render at once
        #          (avoids the freeze). Page state persists across Apply.
        _PER_PAGE = 50
        _total = len(kept)
        _num_pages = max(1, (_total + _PER_PAGE - 1) // _PER_PAGE)
        # current page (clamp to valid range; reset to 1 if the match set changed size)
        _pg = globals().get('_opt_page', 1)
        if globals().get('_opt_page_total') != _total:
            _pg = 1  # match set changed (new filter) → start at page 1
        _pg = max(1, min(_pg, _num_pages))
        globals()['_opt_page'] = _pg
        globals()['_opt_page_total'] = _total
        _start = (_pg - 1) * _PER_PAGE
        _end = min(_start + _PER_PAGE, _total)

        # banner
        tk.Label(frame,
                 text=(f"✓ Filter applied — {_total} matching variants  "
                       f"(page {_pg} of {_num_pages}, showing {_start + 1}–{_end})"),
                 bg="#e8f5e9", fg="#1b5e20", font=("Segoe UI", 10, "bold"),
        ...
        def _build_pager(parent):
            if _num_pages <= 1:
                return
            bar = tk.Frame(parent, bg=BG); bar.pack(fill="x", padx=12, pady=(0, 6))
            tk.Button(bar, text="‹ Prev", state=("normal" if _pg > 1 else "disabled"),
                      command=lambda: _go_page(_pg - 1), font=("Segoe UI", 9),
                      bg="#34495e", fg="white", relief="flat", padx=8, cursor="hand2"
                      ).pack(side="left", padx=2)
            # page number buttons (windowed if many pages)
            _lo = max(1, _pg - 4); _hi = min(_num_pages, _pg + 4)
            if _lo > 1:
                tk.Button(bar, text="1", command=lambda: _go_page(1),
                          font=("Segoe UI", 9), relief="flat", bg="#ecf0f1",
                          padx=6, cursor="hand2").pack(side="left", padx=1)
                if _lo > 2:
                    tk.Label(bar, text="…", bg=BG, fg="#888").pack(side="left", padx=2)
            for _p in range(_lo, _hi + 1):
                _cur = (_p == _pg)
                tk.Button(bar, text=str(_p), command=(lambda pp=_p: _go_page(pp)),
                          font=("Segoe UI", 9, "bold" if _cur else "normal"),
                          bg=("#2980b9" if _cur else "#ecf0f1"),
                          fg=("white" if _cur else "#333"),
                          relief="flat", padx=7, cursor="hand2").pack(side="left", padx=1)
            if _hi < _num_pages:
                if _hi < _num_pages - 1:
                    tk.Label(bar, text="…", bg=BG, fg="#888").pack(side="left", padx=2)
                tk.Button(bar, text=str(_num_pages), command=lambda: _go_page(_num_pages),
                          font=("Segoe UI", 9), relief="flat", bg="#ecf0f1",
                          padx=6, cursor="hand2").pack(side="left", padx=1)
            tk.Button(bar, text="Next ›", state=("normal" if _pg < _num_pages else "disabled"),
                      command=lambda: _go_page(_pg + 1), font=("Segoe UI", 9),
                      bg="#34495e", fg="white", relief="flat", padx=8, cursor="hand2"
                      ).pack(side="left", padx=2)
        _build_pager(frame)

        # render only this page's cards (≤ 50)
        for _i, cand in enumerate(kept[_start:_end]):
            rank = _start + _i + 1
            _stats = cand.get('stats') or {}
            try:
                _render_opt_card(frame, rank, cand, _stats, 1.0, 10000, None, None, 1.0, None)
            except Exception as _ce:
                _ef = tk.Frame(frame, bg="#fff8e1", highlightbackground="#e0c060",
                               highlightthickness=1); _ef.pack(fill="x", padx=12, pady=3)
                tk.Label(_ef, text=f"#{rank} {cand.get('name','?')} — could not render: {_ce}",
                         bg="#fff8e1", fg="#8a6d00", font=("Segoe UI", 8),
                         anchor="w").pack(fill="x", padx=8, pady=4)

        # page navigation bar (bottom too, for convenience)
        _build_pager(frame)

        # status-line notification
        if globals().get('_opt_status_lbl'):
            _opt_status_lbl.configure(
                text=f"✓ Filter applied — {_total} matches, page {_pg}/{_num_pages}", fg=GREEN)
        try:
            _sc = globals().get('_scroll_canvas')
            if _sc:
                _sc.after_idle(lambda: _sc.configure(scrollregion=_sc.bbox("all")))
        except Exception:
            pass
    except Exception:
        import traceback; traceback.print_exc()
...
def build_panel(parent):
    ...
    def _on_strategies_loaded():
        ...
        if not _strategies:
            ...
            _bt_row_n   = 0
            _seen_row_sigs = set()   # CHANGED: June 2026 — skip exact-duplicate grid rows
            for s in _sr_my_rules + _sr_saved + _sr_sep + _sr_others:
                ...
                # CHANGED: June 2026 — drop EXACT duplicate rows (same rule, exit, TF,
                #          direction AND identical stats). Keep the first occurrence.
                #          Separators are never deduped; starred/my-rules rows are kept
                #          (intentional, user-curated) — only auto-listed rows dedup.
                if source != 'separator' and not is_starred and s.get('source') != 'my_rules':
                    _row_sig = (
                        str(rc), str(exit_name),
                        str(s.get('entry_tf') or s.get('entry_timeframe') or ''),
                        str(s.get('direction', '')),
                        int(trades or 0),
                        round(float(wr or 0), 4),
                        round(float(pf or 0), 4),
                        round(float(net or 0), 2),
                        round(float(avg or 0), 4),
                    )
                    if _row_sig in _seen_row_sigs:
                        continue   # exact duplicate — skip
                    _seen_row_sigs.add(_row_sig)
    ...
    tk.Button(opt_filt_row, text="Apply", font=("Segoe UI", 8, "bold"),
              ...
              cursor="hand2", command=lambda: (
                  globals().__setitem__('_opt_page', 1),
                  globals().__setitem__('_opt_page_total', None),
                  _rerender_opt_run_cards())
